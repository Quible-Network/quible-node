...
export class QuibleWallet {
	constructor(public provider: QuibleProvider, public address: string) {}

	prepareCreateQuirkleEvent(params: Pick<CreateQuirkleParams, 'members' | 'proofTtl'>, slug: string = `${Math.random()}`): TransactionEvent {
	...
	async createQuirkle(params: CreateQuirkleParams, slug: string = `${Math.random()}`): Promise<QuirkleRoot> {
	...
	computeQuirkleRootRaw(slug: string): Uint8Array {
		const authorBytes = new Uint8Array(this.address.slice(2).match(/.{1,2}/g)!.map((byte) => parseInt(byte, 16)))
...
export class UnsignedTransaction {
	...
	async sign(signer: (message: Uint8Array) => Promise<string>): Promise<SignedTransaction> {
...
export class SignedTransaction {
	constructor(public events: TransactionEvent[], public signature: string) {}
